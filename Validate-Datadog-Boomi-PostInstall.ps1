...
try {
    ...
    if (Test-Path $agentPath) {
        ...
        if ($ddVersion) {
            Write-Host $ddVersion
        } else {
            Write-Host "[WARN] Unable to detect Datadog version" -ForegroundColor Yellow
        }
    } else {
...
foreach ($p in $paths) {
    if (Test-Path $p) {
        Write-Host "[OK] $p exists" -ForegroundColor Green
    } else {
...
if (Test-Path $logPath) {
    ...
    Write-Host "[WARN] Datadog log file not found" -ForegroundColor Yellow
}
